...
/*   Updated: 2024/03/18 00:34:32 by rrhnizar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Response::handleDirectoryRequest(const std::string& Root_ReqPath, const Location& location)
{
    ...
    if(ResPath.empty() == 0)
	{
        ...
        string  extension = ResPath.substr(pos + 1); // get extension
        ...
        {
            size_t _pos = str_utils::r_find(ResPath, '/');
		    std::map<std::string, std::string> env;
		    env["SCRIPT_NAME"] = ResPath.substr(_pos + 1); //It will be the name of the file
		    env["SCRIPT_FILENAME"] = ResPath; //It will be the path of the file
		    env["PATH_INFO"] = ResPath;
